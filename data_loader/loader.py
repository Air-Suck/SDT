...
class ScriptDataset(Dataset):
    def __init__(self, root="data", dataset="CHINESE", is_train=True, num_img=15):
        ...
        self.content = pickle.load(
            open(os.path.join(data_path, script[dataset][1]), "rb")
        )  # content samples
        self.char_dict = pickle.load(
            open(os.path.join(data_path, "character_dict.pkl"), "rb")
        )
        self.all_writer = pickle.load(
            open(os.path.join(data_path, "writer_dict.pkl"), "rb")
        )
        print(
            "======================================:",
            os.path.join(data_path, "writer_dict.pkl"),
        )
        self.is_train = is_train
        if self.is_train:
            lmdb_path = os.path.join(data_path, "train")  # online characters
            self.img_path = os.path.join(
                data_path, "train_style_samples"
            )  # style samples
    ...
    def __getitem__(self, index):
        ...
        char_img = char_img / 255.0  # Normalize pixel values between 0.0 and 1.0
...
class Online_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        with self.lmdb.begin(write=False) as txn:
            data = pickle.loads(txn.get(str(index).encode("utf-8")))
            character_id, coords, writer_id, coords_gt = (
                data["character_id"],
                data["coordinates"],
                data["writer_id"],
                data["coords_gt"],
    ...
    def collate_fn_(self, batch_data):
        ...
        output = {
            "coords": torch.zeros((bs, max_len, 5)),  # preds -> (x,y,state)
...
class UserDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return {
            "char_img": torch.Tensor(char_img).unsqueeze(0),
            "img_list": torch.Tensor(img_list),
            "char": char,
        }
...
class test_offline_Style_Dataset(Dataset):
    def __init__(self, root=None, is_train=True, num_img=15):
        ...
        if os.path.exists(os.path.join(root, "writer_dict.pkl")):
            self.writer_dict = pickle.load(
                open(os.path.join(root, "writer_dict.pkl"), "rb")
            )
        else:

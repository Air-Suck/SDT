...
def main(opt):
    ...
    if opt.sample_size == "all":
        ...
        batch_samples = (
            int(opt.sample_size) * len(writer_dict) // cfg.TRAIN.IMS_PER_BATCH
        )
    ...
    with torch.no_grad():
        for _ in tqdm.tqdm(range(batch_samples)):
            ...
            if batch_num > batch_samples:
                ...
                data = next(data_iter)
                # prepare input
                coords, coords_len, character_id, writer_id, img_list, char_img = (
                    data["coords"].cuda(),
                ...
                if opt.store_type == "online":
                    for i, pred in enumerate(preds):
                        ...
                        ###############################################
                        # 将 data 写入到 txt 文件中
                        output_file = "output.txt"  # 定义输出文件路径
                        with open(
                            output_file, "a", encoding="utf-8"
                        ) as f:  # 使用追加模式 ("a")
                            f.write(f"Sample {num_count}:\n")  # 写入样本编号
                    ...
                    for i, pred in enumerate(preds):
                        ...
                        try:
                            ...
                            print(
                                "error. %s, %s, %s"
                                % (save_path, str(writer_id[i].item()), character)
                            )
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cfg",
        dest="cfg_file",
        default="configs/CHINESE_CASIA.yml",
        help="Config file for training (and optionally testing)",
    )
    parser.add_argument(
        "--dir",
        dest="save_dir",
        default="Generated/Chinese",

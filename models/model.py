...
class SDT_Generator(nn.Module):
    def __init__(
        ...
        self.writer_head = TransformerEncoder(
            encoder_layer, num_head_layers, writer_norm
        )
    ...
    def random_double_sampling(self, x, ratio=0.25):
        ...
        ids_keep_anchor, ids_keep_pos = (
            ids_shuffle[:, :, :anchor_tokens],
            ids_shuffle[:, :, anchor_tokens:pos_tokens],
        )
    ...
    def forward(self, style_imgs, seq, char_img):
        ...
        style_embe = style_embe.view(batch_size * num_imgs, 512, -1).permute(
            2, 0, 1
        )  # [4, B*2N, C:512]
        ...
        writer_memory = rearrange(
            writer_memory, "t (b p n) c -> t (p b) n c", b=batch_size, p=2, n=anchor_num
        )  # [4, 2*B, N, C]
        glyph_memory = rearrange(
            glyph_memory, "t (b p n) c -> t (p b) n c", b=batch_size, p=2, n=anchor_num
        )  # [4, 2*B, N, C]
        ...
        tgt = torch.cat(
            (char_emb, seq_emb), 0
        )  # [1+T], put the content token as the first token
    ...
    def inference(self, style_imgs, char_img, max_len):
        ...
        FEAT_ST = style_embe.reshape(batch_size * num_imgs, 512, -1).permute(
            2, 0, 1
        )  # [4, B*N, C]
